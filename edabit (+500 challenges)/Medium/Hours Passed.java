...
public class Times {
	public static String hoursPassed(String... s) {
		int[] t = Stream.of(s).map(e -> e.split(":00 ")).flatMap(Stream::of).mapToInt(k -> k
			.matches("\\d+") ? k.equals("12") ? 0 : Integer.parseInt(k) : k.equals("PM") ? 12 : 0).toArray();
		return t[0] == t[2] && t[1] == t[3] ? "No time has passed." : t[2]+t[3]-t[0]-t[1] + " hours";
	}
}
...
public class Times {
	public static String hoursPassed(String t1, String t2) {
		...
		return hoursPassed > 0 ? hoursPassed + " hours" : "No time has passed.";
	}
}
